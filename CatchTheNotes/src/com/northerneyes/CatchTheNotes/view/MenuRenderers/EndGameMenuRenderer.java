...
public class EndGameMenuRenderer  implements IRenderer {
    ...
    public void render(SpriteBatch spriteBatch) {
        ...
        if(menu.Finished)
        {
            ...
            textRenderer.render(spriteBatch);
            if(menu.getMedal() > 0)
                renderMedal(spriteBatch, menu.MedalPosition.x, menu.MedalPosition.y + 5);
            return;
        }
    ...
    public void renderMedal(SpriteBatch spriteBatch, float x, float y)
    {
        ...
        spriteBatch.draw(region, x*coeff - width/2,
                (y - 5)*ppuY - height/2, menu.MedalSize*ppuX,  menu.MedalSize*ppuY);
        spriteBatch.setColor(1);
        spriteBatch.end();
        menu.MedalText.Position.set(x, y- 5);
