...
public class MainMenu implements IEntity{
    ...
    public void startFade(TweenCallback listener) {
        Timeline.createSequence()
                ...
                .beginParallel()
                    .push(Tween.to(AppNameTextColor, ColorAccessor.OPACITY, 1f).target(0).ease(Cubic.IN))
                    .push(Tween.to(PlayTextColor, ColorAccessor.OPACITY, 1f).target(0).ease(Cubic.IN))
                    .push(Tween.to(SongTextColor, ColorAccessor.OPACITY, 1f).target(0).ease(Cubic.IN))
                    .push(Tween.to(btnColor, ColorAccessor.OPACITY, 1f).target(0).ease(Cubic.IN))
                    .push(Tween.to(btnHoverColor, ColorAccessor.OPACITY, 1f).target(0).ease(Cubic.IN))
                    .push(Tween.to(btnPressedColor, ColorAccessor.OPACITY, 1f).target(0).ease(Cubic.IN))
                .end()
